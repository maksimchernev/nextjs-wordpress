...
const ChooseBrand = ({brands}) => {
    ...
    return (
        <div id="brands" className="my-10 sm:my-20 lg:my-40">
            <div className="flex justify-content flex-wrap py-3 container mx-auto px-2" >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5 justify-items-center lg:mt-7 w-full">
                    { brands.length ? brands.map( brand => {
                        const img = brand?.image ?? {}
                            return (
                                <div className="w-full px-5 sm:px-0" key={brand.id}> 
                                    <Link 
