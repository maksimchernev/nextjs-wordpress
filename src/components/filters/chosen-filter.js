...
const ChosenFilter = ({attribute, filters, series}) => {
    ...
    return (
        <div className="w-full flex flex-wrap relative py-1 lg:py-2 lg:mt-3 filter-card px-10 sm:px-4 justify-between lg:bg-brand-yellow bg-opacity-85">
            <p className="flex justify-between  text-brand-gray3E my-0 lg:my-1 font-sf-pro-display-medium text-base"><span dangerouslySetInnerHTML={{ __html: sanitize(sanitizeTags(attribute.name+':'))}}></span></p>
